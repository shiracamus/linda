(function() {
  var Client, Linda, Tuple, TupleSpace, debug, events, fs, http, path, socketio, url,
  ...
  Linda = (function(_super) {
    ...
    function Linda() {
      var _this = this;
      this.spaces = {};
      fs.readFile(path.join(__dirname, 'linda-socketio-client.js'), function(err, data) {
        if (err) {
    ...
    Linda.prototype.listen = function(opts) {
      ...
      this.io.sockets.on('connection', function(socket) {
        var cids, info, watch_cids;
        cids = {};
        info = {
          from: socket.handshake.headers['x-forwarded-for'] || socket.handshake.address.address
        };
